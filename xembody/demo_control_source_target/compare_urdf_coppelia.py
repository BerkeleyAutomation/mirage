...
def setup_robot_environment(tip_name):
    ...
    env = Environment(
        ...
        robot_setup=robot_type, desired_tip_name=tip_name)
...
def execute_agent(agent, task: TaskEnvironment, task_recorder: TaskRecorder):
    ...
    ik_solver = TracIKSolver(
                    "/home/lawrence/Downloads/panda9/panda.urdf",
                    "robot_base",
                    task._robot.arm._ik_tip.get_name().replace('_', ''),
                )
    obs = None
    # import pdb; pdb.set_trace()
    descriptions, obs = task.reset()
    for i in range(training_steps):
        random_angles = np.random.normal(0.0, 0.5, size=(7,))
        print("Set angles: ", random_angles)
        task._task.robot.arm.set_joint_positions(random_angles, disable_dynamics=True)
        received_joints = task._task.robot.arm.get_joint_positions()
        print("Get angles: ", received_joints)
        num_joints = int(task._robot.arm._ik_tip.get_name()[10])
        print("URDF FK: ", T.mat2pose(ik_solver.fk(received_joints[:num_joints])))
        print("Gripper pose: ", task._scene.get_observation().gripper_pose)
            # print(descriptions)
...
def main():
    tips = ['Panda_link7_respondable']
    for tip in tips:
        source_robot_env = setup_robot_environment(tip)
