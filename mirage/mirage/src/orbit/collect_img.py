...
python3 collect_img.py --agent /home/lawrence/xembody/Orbit/source/standalone/workflows/rsl_rl/models/image_based_multihead_policy_40.pth --task Isaac-Lift-Visual-v0  --n_rollouts 1 --seeds 0 --connection --passive
"""
...
class Robot:
    def __init__(self, robot_name=None, ckpt_path=None, video_path=None, rollout_horizon=None, seed=None, task_name=None, env_cfg=None, is_target=False):
        ...
        if self.env_cfg is None:
            self.env_cfg = parse_env_cfg(self.task, use_gpu=False, num_envs=1)

        self.is_target = is_target
        if self.is_target:
            self.env_cfg.control.control_type = "inverse_kinematics"
            self.env_cfg.control.inverse_kinematics.command_type = "position_abs"
        ...
        self.cam_config = PinholeCameraCfg(
                sensor_tick=0,
                height=84,
                width=84,
                data_types=["rgb", "semantic_segmentation", "distance_to_image_plane"],
                usd_params=PinholeCameraCfg.UsdCameraCfg(
                    focal_length=101.39696962, focus_distance=400.0, horizontal_aperture=84, clipping_range=(0.1, 1.0e5)
                )
        )
        self.renderer = OrbitRenderer(robot_path="/World/envs/env_0/Robot", device="cuda", pinhole_camera_config=self.cam_config, resolution=[84, 84])
    ...
    def initialize_robot(self):
        intrinsics = np.array([[101.39696962,   0.,          42.        ],
                                [  0.,         101.39696962,  42.        ],
                                [  0.,           0.,           1.        ]])
        translation = np.array([0.5, 0, 1.35]) - np.array([-0.56, 0, 0.912])
        rotation = np.array( [[ 0. ,         0.70614784, -0.70806442        ],
                                [ 1.    ,      0.,          0.               ],
                                [ 0. ,        -0.70806442, -0.70614784    ]])
        import scipy.spatial.transform as tf
        quat = tf.Rotation.from_matrix(rotation).as_quat()
        self.renderer.initialize(translation=translation, rot_quat=quat, intrinsics=intrinsics)
        self.obs = self.env.reset()
        
    def compute_pose_error(self, target_pose):
        starting_pose = self.compute_eef_pose()[:3]
    ...
    def drive_robot_to_target_pose(self, target_pose=None, tracking_error_threshold=0.003, num_iter_max=5):    
        error, starting_pose = self.compute_pose_error(target_pose[:3])
        num_iters = 0
...
class SourceRobot(Robot):
    ...
    def rollout_robot(self, video_skip=5, return_obs=False, set_object_state=True, set_robot_pose=False, tracking_error_threshold=0.003, num_iter_max=100, target_robot_delta_action=False, demo_index=0):
        ...
        for step_i in range(self.rollout_horizon):
            ...
            action = torch.tensor([-0.12760899 + 0.56,  0.01481073,  0.99493895 - 0.912, 0.014093  ,  0.0292269 , -0.00303507, 0.99946884,  1], dtype=torch.float32, device=self.env.device)
            # action = action.squeeze().unsqueeze(0) # add batch dimension
            self.drive_robot_to_target_pose(action[:7].detach().cpu().numpy(), tracking_error_threshold=0.001, num_iter_max=10)
            # action, r, done, success = self.step(action, use_delta=False, blocking=True)
            # if success:
            #     has_succeeded = True
            ...
            image = self.renderer.render_current_view().rgb.cpu().numpy()[:, :, :3]
            breakpoint()
            import cv2
            cv2.imwrite("source_robot_{}.png".format(step_i), cv2.cvtColor(image, cv2.COLOR_RGB2BGR))
...
if __name__ == "__main__":
    ...
    source_robot.rollout_robot(video_skip=args.video_skip, set_object_state=False, set_robot_pose=True, tracking_error_threshold=args.tracking_error_threshold, num_iter_max=args.num_iter_max)
