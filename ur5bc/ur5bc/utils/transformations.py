...
def convert_rig_to_euler(rigid_transf):
    #convert rigid transform to euler (6D)
    rigid_transf_transl = rigid_transf.translation
    rigid_transf_qwxyz = rigid_transf.quaternion
    rigid_transf_qxyzw = np.concatenate((rigid_transf_qwxyz[1:], rigid_transf_qwxyz[[0]]))
    rigid_transf_euler = quat_to_euler(rigid_transf_qxyzw)
    euler_transf = np.concatenate((rigid_transf_transl, rigid_transf_euler))
    return euler_transf

def convert_euler_to_rig(euler):
    # convert 6D Euler to rigid transform
    trans = euler[:3]
    #convert euler to quat
    angles_quat = euler_to_quat(euler[3:]) # q_xyzw
